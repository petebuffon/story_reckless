...
def anchor_upload(episode, input_path):
    ...
    time.sleep(5)
    save_changes = wait.until(element_to_be_clickable((By.XPATH, '//button[normalize-space()="Save episode"]')))
    save_changes.click()
    form = wait.until(presence_of_element_located((By.TAG_NAME, "form")))
    print("adding metadata")
    form.find_element(By.XPATH, '//button[normalize-space()="Switch to HTML"]').click()
    ...
    radio = driver.find_element(By.ID, "podcastEpisodeIsExplicit-true")
    driver.execute_script("arguments[0].click();", radio)
    publish_now = wait.until(element_to_be_clickable((By.XPATH, '//button[normalize-space()="Publish now"]')))
    publish_now.click()
